 ...
 *      next round is live immediately (seeded from the 2.5% reserve) ΓÇö no claim is needed
...
contract RacksGame is Ownable, ReentrancyGuard {
    ...
    ///      the first bid ΓÇö which is what starts the new round's countdown. No funds move
    ...
    ///         any forfeited shares. Money does not move here ΓÇö only when the winner claims.
    ...
    function _sweepExpired() internal {
        while (nextPendingRound < round) {
            ...
            if (deadline == 0) {
                ...
                // First still-live claim ΓÇö stop here.
                break;
            }
        }
    }
